...
export class CupBracketDisplayComponent implements OnInit {
  ...
  loadBracket(): void {
    ...
    this.cupService.getMatchesByRound(this.cupId).subscribe({
      next: (data) => {
        ...
        // Check if the tournament has a winner (final match is completed)
        this.checkForTournamentWinner()
  ...
  checkForTournamentWinner(): void {
    ...
    if (finalRound && finalRound.matches.length > 0) {
      ...
      if (
        finalMatch.winner &&
        (finalMatch.statusMatch?.toLowerCase() === "completed" ||
          (finalMatch.goals1 !== null && finalMatch.goals2 !== null))
      ) {
