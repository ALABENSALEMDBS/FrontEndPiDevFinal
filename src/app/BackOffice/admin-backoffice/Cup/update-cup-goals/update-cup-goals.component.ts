...
export class UpdateCupGoalsComponent implements OnInit {
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      this.uploadedImage = new File([this.uploadedImage], this.uploadedImage.name, { type: this.uploadedImage.type })
  ...
  updateScoreFromImage(): void {
    ...
    const modifiedFile = new File([this.uploadedImage], newFileName, { type: this.uploadedImage.type })
